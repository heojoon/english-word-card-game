...
3. Write explicit animation beats for every frame. For six-frame attacks:
   - attack: ready → anticipation → charge → contact/release → follow-through → recovery;
...
4. Generate a full strip first only when the entire character, prop, and effect can remain well inside each slot. Require exact slot count, one complete character per slot, stable scale/ground line, one direction, and generous transparent gutters.
...
7. Assemble the runtime strip only from independently verified frames. Use `scripts/assemble_strip.sh` when FFmpeg is available. The spaced master must use slots at least 3× the source/runtime slot width when extraction safety is requested.
...
- Attack anticipation, peak action, and recovery read at gameplay size. Hit animation must instead read as notice, impact, and complete knockdown at gameplay size.
