...
- Android 테스트 APK: https://github.com/heojoon/english-word-card-game/releases/tag/android-v1.0.2-beta.3
- Backend / DB: Supabase
...
- Release: https://github.com/heojoon/english-word-card-game/releases/tag/android-v1.0.2-beta.3
- Direct APK: https://github.com/heojoon/english-word-card-game/releases/download/android-v1.0.2-beta.3/app-debug.apk
- SHA-256: `6c152481e9b8183b3eb6d2ebdfb4f92fb4ee336007745110628f47ab3d7cde3e`
...
`보랏빛 결정 궁수`는 여성 궁수 전용 스킨입니다. 상점 가격은 2,000 크리스털이며, 여성 궁수 캐릭터로만 구매하고 `Skin` 장비 슬롯에 장착할 수 있습니다. 장착하면 기본 초상화 대신 스킨 외형이 홈·장비·전투에 적용됩니다.
...
`index.html`의 대문 화면은 Supabase 아이디·비밀번호 로그인/회원가입과 `손님으로 시작` 입장을 제공한다. 신규 아이디는 정규화 후 SHA-256 기반 내부 인증 주소로 변환되며, 이메일 인증 없이 가입 즉시 로그인된다. 내부 주소는 UI에 노출하지 않고 실제 연락용 이메일은 향후 프로필 정보로 별도 등록한다. 이전에 만든 이메일 계정은 기존 이메일로 계속 로그인할 수 있다. 로그인 세션은 Supabase JS가 브라우저에 유지하며, Guest 선택은 현재 브라우저 탭의 `sessionStorage`에만 유지된다. 이 단계의 인증은 게임 입장 게이트이며, 기존 닉네임 기반 캐릭터·점수·재화의 소유권은 아직 `auth.uid()`와 연결되지 않았다.
