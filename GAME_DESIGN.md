...
- 모든 게임 화면의 우측 상단에서 프로필에 진입할 수 있다.
- 프로필에서는 로그인 아이디, 연동 이메일, 계정 권한과 역할별 사용 가능 기능을 확인한다.
- 연동 이메일은 로그인에 사용하는 내부 인증 이메일과 분리해 저장한다.
- 이메일 연동은 10분간 유효한 6자리 문자·숫자 인증코드 입력 후 완료된다. 인증코드는 평문으로 저장하지 않는다.
- 이메일 인증을 처음 완료한 계정에는 계정 단위 보상 200 크리스털을 한 번만 지급한다.
- `heojoon48@gmail.com` 인증 계정은 Admin으로 고정하고, 그 외 인증 계정은 Student와 Teacher 중 역할을 선택할 수 있다.
- 인증된 이메일을 해제하면 주소와 인증 이력은 보존하고 `contact_email_active`만 비활성화한다. Admin 고정 계정을 제외한 일반 계정은 Student로 돌아간다.
- 친구 추가, 친구 맺기, 내 그룹은 같은 프로필 화면에서 확장하되 기능이 준비되기 전에는 비활성 상태를 명확히 표시한다.
- 게스트는 계정 정보를 저장할 수 없으며 프로필에서 로그인 또는 회원가입 화면으로 이동할 수 있다.
...
로그인 후에는 항상 계정에 속한 캐릭터 생성/선택 화면으로 진입한다. 첫 캐릭터는 무료로 생성하며, 두 번째 캐릭터부터는 계정이 보유한 **1,000 크리스털**로 일회용 캐릭터 추가권을 먼저 구매해야 한다. 추가권은 캐릭터 생성이 완료될 때 한 장 소모된다.

- 캐릭터 이름은 로그인 아이디와 별개이며 캐릭터마다 하나씩 정한다.
- 캐릭터 선택 카드에서 직업별 기본 능력치와 특수기술을 비교할 수 있다.
- 선택한 캐릭터로 홈과 던전에 진입한다. 기록과 장비는 캐릭터별로 유지하지만 크리스털은 계정의 모든 캐릭터가 하나의 지갑을 공유한다.
- 추가권 구매와 소모는 서버에서 계정 소유권과 잔액을 검증하고 원자적으로 처리한다.
...
정답률의 모수는 스테이지에 입장한 뒤 실제로 해결한 문제 수다. 오답 또는 시간 초과로 스테이지가 종료될 때 아직 해결하지 못한 해당 문제와 이후 미출제 문제는 모수에서 제외한다.
...
클리어 직후 가장 먼저 표시되는 화면은 전투 완료와 학습 성과를 빠르게 확인하는 보상 전환 구간이다.

- 화면은 짧은 페이드인으로 표시한다.
- `STAGE CLEAR`를 화면의 가장 큰 영문 제목으로 표시한다.
- 트로피 이미지는 사용하지 않는다. 정보 위계와 크리스털 퀘스트의 재질 표현만으로 성취감을 전달한다.
- `클리어 타임`, `푼 문제`, `획득 크리스털` 성적 카드 3개를 동일한 행에 배치한다.
- 모바일 세로 화면에서도 세 카드는 줄바꿈하지 않으며, 글자와 여백을 조절해 수치가 잘리지 않게 한다.
- 성적 카드는 화면이 나타난 직후 왼쪽부터 오른쪽 순서로 빠르게 한 장씩 뒤집힌다.
- 카드 플립은 약 0.42초이며 각 카드 시작 간격은 약 0.12초로 한다. 마지막에는 모든 카드가 읽을 수 있는 정면 상태로 유지된다.
- 기록 저장 성공 또는 실패 상태를 성적 카드 아래에 텍스트로 표시한다.
- 하단의 주 행동은 `보상 상자 확인하기`이며 다음 보상 단계로 이동한다.
- `prefers-reduced-motion` 환경에서는 지연과 회전을 제거하고 즉시 읽을 수 있는 정적 결과를 표시한다.

이 화면은 성적 확인을 보상 루프의 첫 피드백으로 사용한다. 장식보다 플레이 결과를 먼저 읽게 하고, 이어지는 보물상자 연출을 두 번째 보상 순간으로 분리한다.

#### 보물상자 상호작용 및 효과음

- 보물상자는 총 3회 터치하여 연다. 각 유효한 터치는 시각적 흔들림, 햅틱 피드백, 짧은 효과음을 한 번씩 함께 발생시킨다.
- 1·2·3번째 터치 효과음은 짧은 상자 타격음과 맑은 차임을 조합하며, 터치 단계가 올라갈수록 음정을 높여 개봉 진행을 청각적으로 전달한다.
- 세 번째 터치는 앞선 터치보다 강한 개봉음과 성공 햅틱을 사용하고, 중복 입력은 잠가 동일 효과가 반복되지 않게 한다.
- 보상 처리에 성공하여 크리스털이 화면에 등장하는 순간에는 상자 터치음과 구분되는 상승형 보상 차임을 한 번 재생한다.
- 보상 요청이 실패하거나 크리스털 공개 화면으로 전환되지 않은 경우에는 보상 차임을 재생하지 않는다.
- 효과음은 브라우저의 사용자 입력 이후 활성화되는 Web Audio API를 사용하며, 오디오를 지원하지 않는 환경에서도 보상 진행 자체는 중단되지 않는다.
...
아이템의 확정 방향은 **꾸미기 중심 + 학습을 방해하지 않는 가벼운 효과**다. 가벼운 효과는 **펫과 오라 슬롯에만** 적용하며, 머리·몸·무기·등은 외형과 수집 중심으로 둔다. 영어 단어 풀이가 전투의 중심이며 장비가 정답을 대신하거나 필수 학습 콘텐츠 접근을 막지 않는다. 펫 10종·오라 10종의 이름과 기본 디자인은 등급이 바뀌어도 동일하며, 각 아이템에 다섯 등급(총 100개 등급 변형)을 둔다. 등급별 효과 배정 초안과 현행 구현의 차이는 `ITEM_DESIGN.md`에서 관리한다. 이 카탈로그는 아직 게임에 적용되지 않았다.
...
`Skin`은 기본 외형 전체를 교체하는 꾸미기 슬롯이며 능력치를 제공하지 않는다. `보랏빛 결정 궁수` 스킨은 여성 궁수 캐릭터 전용이고 가격은 2,000 크리스털이다. 해당 캐릭터만 구매 및 장착할 수 있으며, 해제하면 기본 여성 궁수 외형으로 돌아간다.

`크리스털 로즈 권투사` 스킨은 여성 권투사 전용이고 가격은 2,000 크리스털이다. 장착 시 프로필 외형뿐 아니라 전투의 공격 및 피격/쓰러짐 스프라이트도 스킨 전용 애니메이션으로 교체되며, 해제하면 기본 여성 권투사 외형과 애니메이션으로 돌아간다.
