...
게임의 비주얼은 **고전 아케이드 판타지 액션 RPG의 감성**을 가진 도트 그래픽으로 구성한다.

참고 감성:

- 1990년대 판타지 횡스크롤 액션 게임
- 강한 검과 마법 연출
- 던전, 성, 숲, 동굴, 묘지, 성채 배경
- 도트 캐릭터
- 몬스터 처치 연출
...
상용화 시에는 기존 게임 IP의 캐릭터, UI, 몬스터, 배경을 그대로 복제하지 않고 **독자적 캐릭터/세계관/아트**를 사용한다.
...
상용 버전에서는 현재의 고정 플레이어 선택 방식(`율이`, `아빠`, `손님`)을 실제 계정 시스템으로 변경한다.
...
현재 고정 플레이어 선택을 실제 사용자 시스템으로 전환한다.
