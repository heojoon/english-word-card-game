(() => {
  ...
  let page='home',filter='item',world=0,stage=0,run=null,timer=null,nextTimer=null,toastTimer=null;
  const save=()=>{try{localStorage.setItem(KEY,JSON.stringify(state));}catch{}};
  const character=()=>classes[state.character]||classes[0];
  const imagePath=()=>`assets/avatars/${character().paths[state.gender===1?1:0]}?v=20260911-hq`;
  ...
  function shop(){const visible=items.filter(item=>(item.slot==='reward'?'reward':item.slot==='skin'?'skin':'item')===filter);return `<div class="tabs" aria-label="상점 카테고리">${[['item','아이템'],['skin','스킨'],['reward','보상']].map(([key,label])=>`<button data-action="filter" data-filter="${key}" class="${filter===key?'active':''}" aria-pressed="${filter===key}">${label}</button>`).join('')}</div><div class="items catalog">${visible.map(i=>itemCard(i)).join('')||'<div class="empty-state">이 카테고리에 판매 중인 상품이 없어요.</div>'}</div><button class="secondary" style="width:100%;margin-top:15px" data-action="requests">보상 신청 내역 (${state.requests.length}) →</button>`;}
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button[data-action]');if(!b||b.disabled)return;const a=b.dataset.action,id=b.dataset.id;
    ...
    else if(a==='slot'){const owned=items.find(i=>i.slot===b.dataset.slot&&state.owned.includes(i.id));if(owned)showItem(owned.id);else{filter=b.dataset.slot==='skin'?'skin':'item';go('shop');toast('이 슬롯에 어울리는 아이템을 골라 보세요');}}
