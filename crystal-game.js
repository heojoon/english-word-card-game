(async () => {
  ...
  function answer(index){
    ...
    if(ok){run.correct++;const gain=earnedCoins(run.correct)-earnedCoins(run.correct-1);document.dispatchEvent(new CustomEvent('wordoria:haptic',{detail:{kind:'success'}}));$('arena').classList.add('hit');$('arena-feedback').textContent=`${run.correct} COMBO! ◆ +${gain}`;$('battle-help').textContent=`정답! ${q.entry[0]} = ${q.entry[2]}`;speak(q.entry[0]);run.feedbackPending=true;nextTimer=setTimeout(continueAfterFeedback,showBattleReward(q,gain));return;}
  ...
  function showItem(id){const item=itemById(id);if(!item)return;const owned=itemOwned(item),eq=Object.values(equippedMap()).some(v=>String(v)===String(id)),gear=item.category==='avatar';modal(`<div class="eyebrow">${gear?`${rarityNames[item.rarity]||'일반'} · ${'★'.repeat(Number(item.stars||1))}`:'REAL WORLD REWARD'}</div><div class="item-art item-detail-art" ${gear?`data-rarity="${esc(item.rarity||'normal')}"`:''}>${itemArt(item)}</div><h2>${esc(item.name)}</h2><p>${esc(item.description)}</p>${gear?`<div class="item-detail-stat"><span>장착 능력치</span><b>${esc(String(item.stat_key||'').toUpperCase())} +${num(item.stat_value)}</b></div>`:''}<div class="row" style="margin-top:18px"><b>${num(item.price)} ◆</b><span class="small muted">보유 ${num(selectedCharacter.coins)} ◆</span></div><button class="primary" data-action="${owned?'equip':'buy'}" data-id="${item.id}" ${!owned&&selectedCharacter.coins<item.price?'disabled':''}>${owned?(eq?'장착 해제':'장착하기'):(selectedCharacter.coins<item.price?'크리스털이 부족해요':item.category==='gift'?'보상 신청하기':'구매하기')}</button>`);}
