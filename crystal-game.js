(async () => {
  ...
  const imagePath = c => `assets/avatars/${characterDef(c).paths[variantOf(c)]}?v=20260911-hq`;
  ...
  function renderCharacterForm(){if($('character-name'))newCharacterName=$('character-name').value;if(accountMode){render();return;}const d=classDefs[newClass];modal(`<div class="eyebrow">CREATE YOUR HERO</div><h2>${esc(player)}님의 새 모험가</h2><label class="form-label" for="character-name">캐릭터 이름</label><input id="character-name" class="field" maxlength="16" value="${esc(newCharacterName)}" placeholder="예: 불꽃검 율이"><label class="form-label">직업</label><div class="tabs">${Object.entries(classDefs).map(([id,c])=>`<button class="${id===newClass?'active':''}" data-action="class" data-value="${id}">${c.label}</button>`).join('')}</div><p>${d.trait}</p><label class="form-label">기본 외형</label><div class="choice-grid">${['male','female'].map(v=>`<button class="choice-card ${v===newVariant?'active':''}" data-action="variant" data-value="${v}"><img src="assets/avatars/${d.paths[v]}?v=20260911-hq" alt="">${v==='male'?'남성':'여성'} 캐릭터</button>`).join('')}</div><label class="form-label">오라 색상</label><div class="accent-row">${['violet','red','blue','green','gold'].map(v=>`<button class="accent-choice ${v===newAccent?'active':''}" data-action="accent" data-value="${v}" aria-label="${v}"></button>`).join('')}</div><button class="primary" data-action="create-character">캐릭터 생성 →</button>`);}
  function showItem(id){const item=itemById(id);if(!item)return;const owned=itemOwned(item),eq=Object.values(equippedMap()).some(v=>String(v)===String(id)),gear=item.category==='avatar',balance=walletBalance();modal(`<div class="eyebrow">${gear?`${rarityNames[item.rarity]||'일반'} · ${'★'.repeat(Number(item.stars||1))}`:'REAL WORLD REWARD'}</div><div class="item-art item-detail-art" ${gear?`data-rarity="${esc(item.rarity||'normal')}"`:''}>${itemArt(item)}</div><h2>${esc(item.name)}</h2><p>${esc(item.description)}</p>${gear?`<div class="item-detail-stat"><span>장착 능력치</span><b>${esc(String(item.stat_key||'').toUpperCase())} +${num(item.stat_value)}</b></div>`:''}<div class="row" style="margin-top:18px"><b>${num(item.price)} ◆</b><span class="small muted">계정 보유 ${num(balance)} ◆</span></div><button class="primary" data-action="${owned?'equip':'buy'}" data-id="${item.id}" ${!owned&&balance<item.price?'disabled':''}>${owned?(eq?'장착 해제':'장착하기'):(balance<item.price?'크리스털이 부족해요':item.category==='gift'?'보상 신청하기':'구매하기')}</button>`);}
  ...
  async function purchase(item){if(!item||walletBalance()<item.price)return;try{if(demo){selectedCharacter.coins-=item.price;if(item.category==='avatar')demoState.inventory.push({character_id:selectedCharacter.id,item_id:item.id});else demoState.redemptions.unshift({id:Date.now(),character_id:selectedCharacter.id,item_id:item.id,status:'pending',price_paid:item.price,created_at:new Date().toISOString()});saveDemo();}else{const rows=await rpc('purchase_shop_item',{p_character_id:selectedCharacter.id,p_item_id:item.id});if(rows?.[0]){if(accountMode)accountCrystals=Number(rows[0].new_balance);else selectedCharacter.coins=rows[0].new_balance;}}await loadCharacterExtras();closeDialog();render();toast(item.category==='gift'?'보상 신청이 접수되었어요':'구매 완료! 장비함에서 장착해 보세요');}catch(error){console.error(error);toast(String(error.message).includes('not enough')?'계정 크리스털이 부족해요':'구매하지 못했습니다');}}
