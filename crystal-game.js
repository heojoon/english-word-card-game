(async () => {
  ...
  let characterView = localStorage.getItem('wordoriaCharacterView') || 'ring', characterArmedId = null;
  if(!['ring','deck','stage'].includes(characterView))characterView='ring';
  ...
  function characterSelectMarkup(){
    ...
    const rail=characters.map(c=>{const active=c.id===selectedCharacter?.id;return `<button class="select-hero-chip" data-action="select-character" data-id="${c.id}" aria-pressed="${active}"><img src="${imagePath(c)}" alt=""><span>${esc(c.name)}</span></button>`;}).join('');
    const d=characterDef(selectedCharacter),s=stats(),armed=characterArmedId===selectedCharacter?.id;
    return `<section class="character-select" data-view="${characterView}"><div class="character-card-stage" aria-label="보유 캐릭터 입체 카드"><div class="character-card-orbit">${cards}</div><div class="character-stage-glow" aria-hidden="true"></div></div><div class="character-chip-rail" aria-label="보유 캐릭터 빠른 선택">${rail}</div><article class="selected-hero-detail"><div class="selected-hero-title"><span class="level">LV.${s.level}</span><div><small>${esc(d.title)} · ${esc(d.label)}</small><h2>${esc(selectedCharacter.name)}</h2></div><b>${armed?'한 번 더 누르기':'READY'}</b></div><div class="selected-hero-skill"><span>✦</span><div><small>SPECIAL SKILL · ${esc(d.skill)}</small><strong>${esc(d.skillDesc)}</strong></div></div>${classStatsMarkup(d)}<button class="primary play-character" data-action="enter-game">${esc(selectedCharacter.name)}로 플레이하기 <span>→</span></button></article><div class="character-view-switch" role="group" aria-label="캐릭터 카드 보기 모양"><span>보기 모양</span>${[['ring','크리스털 링'],['deck','히어로 덱'],['stage','소환 무대']].map(([id,label],index)=>`<button data-action="character-view" data-view="${id}" aria-pressed="${id===characterView}"><i>${index+1}</i><b>${label}</b></button>`).join('')}</div></section>`;
  }
  ...
  function renderCharacterGate(){
    ...
    return `<section class="character-gate"><header class="character-gate-heading"><div><span class="eyebrow">CHOOSE YOUR HERO</span><h1>오늘의 모험가를 선택하세요</h1><p><b>@${esc(loginLabel)}</b>님의 캐릭터 ${characters.length}명 · 카드를 다시 누르면 바로 시작해요</p></div><span class="gate-crystal" title="계정 공용 크리스털">계정 ◆ ${num(walletBalance())}</span></header>${characterSelectMarkup()}<aside class="character-ticket-card"><div class="ticket-icon">＋</div><div><b>캐릭터 추가권</b><p>첫 캐릭터 이후에는 추가권 1장이 필요해요.</p></div>${availableCharacterTickets?`<button class="secondary" data-action="open-character-create">추가권 사용 · ${availableCharacterTickets}장</button>`:`<button class="ticket-buy" data-action="buy-character-ticket" ${canBuy?'':'disabled'}><span>◆ 1,000</span>${canBuy?'추가권 구매':'크리스털 부족'}</button>`}</aside><p class="ticket-note">추가권과 상점 구매는 계정의 공용 크리스털을 사용합니다.</p></section>`;
  }
  function renderHome(){
  ...
  document.addEventListener('click',async event=>{const b=event.target.closest('button[data-action]');if(!b||b.disabled)return;const action=b.dataset.action,id=b.dataset.id;
    ...
    else if(action==='character-view'){characterView=['ring','deck','stage'].includes(b.dataset.view)?b.dataset.view:'ring';localStorage.setItem('wordoriaCharacterView',characterView);render();}
