(() => {
  ...
  function answer(index){
    ...
    if(ok){run.correct++;$('arena').classList.add('hit');$('arena-feedback').textContent=`${run.correct} COMBO! ◆ +${earnedCoins(run.correct)-earnedCoins(run.correct-1)}`;$('battle-help').textContent='정답! 단어의 힘으로 몬스터를 물리쳤어요.';speak(q.entry[0]);}
    else{$('arena').classList.add('wrong');$('arena-feedback').textContent=index<0?'시간 초과!':'아쉬워요!';$('battle-help').textContent=`정답: ${q.entry[0]} = ${q.entry[2]}`;}
