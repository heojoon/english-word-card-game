(async () => {
  ...
  async function loadAll(){
    ...
    if(demo){
      allCharacters=demoState.characters;shopItems=[
        ...
        {id:10,code:'ranger_violet_crystal_skin',name:'보랏빛 결정 궁수',category:'avatar',price:2000,icon:'◆',description:'은보랏빛 트윈테일과 결정 장궁으로 모습을 바꾸는 여성 궁수 전용 스킨입니다.',slot:'skin',rarity:'legendary',stars:5,stat_key:null,stat_value:0,art_path:'assets/avatars/skins/ranger-female-violet-crystal.webp'},
  ...
  function skinEligible(c=selectedCharacter){return c?.class==='ranger'&&variantOf(c)==='female';}
  function renderItemCard(item,ownedView=false){const owned=itemOwned(item),eq=Object.values(equippedMap()).some(id=>String(id)===String(item.id)),gear=item.category==='avatar',skin=isSkin(item);return `<button class="item" data-action="item" data-id="${item.id}" ${gear?`data-rarity="${esc(item.rarity||'normal')}"`:''}>${gear?`<span class="item-rarity">${rarityNames[item.rarity]||'일반'} · ${'★'.repeat(Number(item.stars||1))}</span>`:''}${ownedView?`<span class="item-status">${eq?'장착 중':'보유'}</span>`:''}<span class="item-art">${itemArt(item)}</span><b>${esc(item.name)}</b>${gear?`<span class="item-stat">${skin?'여성 궁수 전용 스킨':`${esc(String(item.stat_key||'').toUpperCase())} +${num(item.stat_value)}`}</span>`:''}<small class="${owned?'owned':''}">${owned?(eq?'✓ 장착 중':'✓ 보유 중'):`${num(item.price)} ◆`}</small></button>`;}
  function renderGear(){
  ...
  function showItem(id){const item=itemById(id);if(!item)return;const owned=itemOwned(item),eq=Object.values(equippedMap()).some(v=>String(v)===String(id)),gear=item.category==='avatar',skin=isSkin(item),eligible=!skin||skinEligible(),balance=walletBalance(),disabled=!owned&&(!eligible||balance<item.price);modal(`<div class="eyebrow">${gear?`${rarityNames[item.rarity]||'일반'} · ${'★'.repeat(Number(item.stars||1))}`:'REAL WORLD REWARD'}</div><div class="item-art item-detail-art" ${gear?`data-rarity="${esc(item.rarity||'normal')}"`:''}>${itemArt(item)}</div><h2>${esc(item.name)}</h2><p>${esc(item.description)}</p>${gear?`<div class="item-detail-stat"><span>${skin?'사용 조건':'장착 능력치'}</span><b>${skin?'여성 궁수 전용':`${esc(String(item.stat_key||'').toUpperCase())} +${num(item.stat_value)}`}</b></div>`:''}<div class="row" style="margin-top:18px"><b>${num(item.price)} ◆</b><span class="small muted">계정 보유 ${num(balance)} ◆</span></div><button class="primary" data-action="${owned?'equip':'buy'}" data-id="${item.id}" ${disabled?'disabled':''}>${owned?(eq?'장착 해제':'장착하기'):(!eligible?'여자 궁수만 구매할 수 있어요':balance<item.price?'크리스털이 부족해요':item.category==='gift'?'보상 신청하기':'구매하기')}</button>`);}
  ...
  async function purchase(item){if(!item||walletBalance()<item.price)return;if(isSkin(item)&&!skinEligible()){toast('여자 궁수 캐릭터만 이 스킨을 구매할 수 있어요');return;}try{if(demo){selectedCharacter.coins-=item.price;if(item.category==='avatar')demoState.inventory.push({character_id:selectedCharacter.id,item_id:item.id});else demoState.redemptions.unshift({id:Date.now(),character_id:selectedCharacter.id,item_id:item.id,status:'pending',price_paid:item.price,created_at:new Date().toISOString()});saveDemo();}else{const rows=await rpc('purchase_shop_item',{p_character_id:selectedCharacter.id,p_item_id:item.id});if(rows?.[0]){if(accountMode)accountCrystals=Number(rows[0].new_balance);else selectedCharacter.coins=rows[0].new_balance;}}await loadCharacterExtras();closeDialog();render();toast(item.category==='gift'?'보상 신청이 접수되었어요':'구매 완료! 장비함에서 장착해 보세요');}catch(error){console.error(error);toast(String(error.message).includes('female ranger')?'여자 궁수 캐릭터만 구매할 수 있어요':String(error.message).includes('not enough')?'계정 크리스털이 부족해요':'구매하지 못했습니다');}}
  async function equip(item){if(!item||!itemOwned(item))return;if(isSkin(item)&&!skinEligible()){toast('여자 궁수 캐릭터만 이 스킨을 장착할 수 있어요');return;}const slot=slotFor(item),map=equippedMap(),isEquipped=String(map[slot])===String(item.id);try{if(demo){if(isEquipped)delete map[slot];else map[slot]=item.id;selectedCharacter.equipped_items=map;saveDemo();}else{await rpc('equip_avatar_slot',{p_character_id:selectedCharacter.id,p_item_id:isEquipped?null:item.id,p_slot:slot});await loadAll();}closeDialog();render();toast(isEquipped?'스킨 장착을 해제했어요':'장착했어요! 캐릭터 모습이 바뀌었어요');}catch(error){console.error(error);toast(String(error.message).includes('female ranger')?'여자 궁수 캐릭터만 장착할 수 있어요':'장비 슬롯 기능을 사용하려면 최신 DB 마이그레이션이 필요합니다');}}
