(async () => {
  ...
  function showNewCharacter(){newClass='warrior';newVariant='male';newAccent='violet';newCharacterName='';if(accountMode){if(characters.length&&!availableCharacterTickets){modal(`<div class="eyebrow">ADD NEW HERO</div><h2>새 캐릭터 추가</h2><p>캐릭터 추가권은 계정 크리스털 1,000개로 구매할 수 있어요.</p><div class="row" style="margin:18px 0"><b>◆ 1,000</b><span class="small muted">보유 ◆ ${num(walletBalance())}</span></div><button class="primary" data-action="buy-character-ticket-and-create" ${walletBalance()<1000?'disabled':''}>${walletBalance()<1000?'크리스털이 부족해요':'추가권 구매 후 만들기'}</button>`);return;}characterCreating=true;go('characters');return;}renderCharacterForm();}
