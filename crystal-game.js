(() => {
  ...
  function combatStats(level){
    const equipped=Object.values(equippedMap()).filter(Boolean).length;
    ...
    const hp=72+level*4+(classBonus.hp||0),mp=24+level*2+(classBonus.mp||0),atk=12+level*2+(classBonus.atk||0)+equipped*2,def=10+level*2+(classBonus.def||0)+equipped*3,luk=5+Math.floor(level/2)+(classBonus.luk||0)+equipped;
    return {hp,mp,atk,def,luk,power:hp*4+mp*2+atk*12+def*10+luk*8,equipped};
  }
  ...
  function renderGear(){
    ...
    const slotButton=(slot,label,art,enabled=true)=>`<button class="gear-status-slot ${eq[slot]?'equipped':'empty'} ${enabled?'':'locked'}" ${enabled?'data-action="slot" data-slot="'+slot+'"':'disabled'} aria-label="${label}${enabled?(eq[slot]?' 장착 중':' 비어 있음'):' 슬롯 준비 중'}"><span>${icon(art)}</span><small>${label}</small>${!enabled?'<i>준비 중</i>':''}</button>`;
    const stat=(label,value,bonus='')=>`<div class="gear-stat"><span>${label}</span><strong>${value}</strong>${bonus?`<small>+${bonus}</small>`:''}</div>`;
    return `<div class="gear-page-title"><div><span>MY ADVENTURER</span><h1>장비와 상태</h1></div><button class="text-btn" data-action="characters">캐릭터 변경 ›</button></div><section class="gear-status-hero"><div class="gear-identity"><span class="level">LV.${s.level}</span><p>${d.title}</p><h2>${esc(selectedCharacter.name)}</h2><small>${d.label} · ${esc(d.trait.split(' · ')[0])}</small></div><div class="gear-stat-rail">${stat('HP',cs.hp)}${stat('MP',cs.mp)}${stat('ATK',cs.atk,cs.equipped*2)}${stat('DEF',cs.def,cs.equipped*3)}${stat('LUK',cs.luk,cs.equipped)}</div><div class="gear-character">${portrait()}<span>전투력 <b>${num(cs.power)}</b></span></div><div class="gear-slot-rail">${slotButton('head','머리','crown')}${slotButton('body','몸','gear',false)}${slotButton('weapon','무기','sword',false)}${slotButton('back','등','cape')}${slotButton('aura','오라','aura')}${slotButton('pet','펫','pet')}</div></section><button class="panel gear-growth" data-action="stats"><span class="row small"><span>다음 레벨까지</span><b>${num(s.exp)} / 1,000 EXP</b></span><span class="xp"><i style="width:${s.exp/10}%"></i></span><small>정답을 맞히면 경험치와 전투력이 함께 올라가요.</small></button><div class="section-title gear-collection-title"><div><span class="eyebrow">COLLECTION</span><h2>내 장비함 <span class="muted small">${inventory.length}</span></h2></div><button class="text-btn" data-action="nav" data-page="shop">상점 가기 ›</button></div>${inventory.length?`<div class="items catalog gear-inventory-grid">${shopItems.filter(itemOwned).map(i=>renderItemCard(i,true)).join('')}</div>`:'<div class="panel empty-state">아직 수집한 장비가 없어요.<br><button class="text-btn" data-action="nav" data-page="shop">상점에서 첫 장비 만나기 →</button></div>'}<div class="sync-note">장착 상태는 캐릭터와 함께 저장됩니다.</div>`;
  }
  function renderShop(){
